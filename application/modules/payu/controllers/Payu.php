...
class Payu extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Instructor', 'Employee', 'Student'))) {
    ...
    public function check1($gross_total, $data2,$status) {

        $amount = $gross_total;
        // $invoice_id = $this->input->get('payment_id');
        //   $payment = $this->finance_model->getPaymentById($invoice_id);
        //  $client_info = $this->customer_model->getCustomerById($payment->customer);
        $courselist = $this->course_model->getCourseById($data2['course']);
        $studentlist = $this->student_model->getStudentById($data2['student']);
 if (!empty($data2['discount'])) {
            $discount = $data2['discount'];
        } else {
            $discount = 0;
        }
        
        $product_info = $data2['batch'] . '-' . $data2['student'].'-'.$discount .'-'.$status;
        $customer_id = $studentlist->id;
        $customer_name = $data2['student_name'];
        $customer_emial = $studentlist->email;
        $customer_mobile = $studentlist->phone;
        $customer_address = $studentlist->address;
